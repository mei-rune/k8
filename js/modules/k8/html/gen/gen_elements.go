...
func main() {
	...
	if parseErr != nil {
		log.Fatal("Could not parse elements.go", parseErr)
	}
	...
	if err != nil {
		log.Fatal("format.Source on generated code failed", err)
	}

	f, err := os.Create("elements_gen.go")
	if err != nil {
		log.Fatal("Unable to create the file 'elements_gen.go'", err)
	}

	if _, err = f.Write(src); err != nil {
		log.Fatal("Unable to write to 'elements_gen.go'", err)
	}

	err = f.Close()
	if err != nil {
		log.Fatal("Unable to close 'elements_gen.go'", err)
	}
}
