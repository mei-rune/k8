...
func getSimpleBuilder(filename, data string, opts ...interface{}) (*Builder, error) {
	var (
		...
		compatibilityMode = gojs.CompatibilityModeBase
	)
	for _, o := range opts {
		switch opt := o.(type) {
		// case vfs.FileSystem:
	...
	builder, err := NewBuilder(&gojs.RuntimeOptions{
		CompatibilityMode: compatibilityMode.String(),
		Env:               env,
	})
	if err != nil {
		return nil, err
...
func TestNewBuilder(t *testing.T) {
	...
	t.Run("Invalid", func(t *testing.T) {
		...
		assert.NotNil(t, err)
		if err != nil {
			assert.Contains(t, err.Error(), "/script.js: Line 1:1 Unexpected token ILLEGAL (and 1 more errors)")
		}
	})
	...
	t.Run("DefaultUndefined", func(t *testing.T) {
		b, err := getSimpleBuilder("/script.js", `export default undefined;`, gojs.CompatibilityModeExtended)
		if err == nil {
			_, err = b.Build(ctx, nil)
		}
		assert.EqualError(t, err, "script must export a default function")
	...
	t.Run("DefaultNull", func(t *testing.T) {
		b, err := getSimpleBuilder("/script.js", `export default null;`, gojs.CompatibilityModeExtended)
		if err == nil {
			_, err = b.Build(ctx, nil)
		}
		assert.EqualError(t, err, "script must export a default function")
	...
	t.Run("DefaultWrongType", func(t *testing.T) {
		b, err := getSimpleBuilder("/script.js", `export default 12345;`, gojs.CompatibilityModeExtended)
		if err == nil {
			_, err = b.Build(ctx, nil)
		}
	...
	t.Run("Minimal", func(t *testing.T) {
		b, err := getSimpleBuilder("/script.js", `export default function() {};`, gojs.CompatibilityModeExtended)
		if err == nil {
			_, err = b.Build(ctx, nil)
		}
	...
	t.Run("CompatibilityMode", func(t *testing.T) {
		t.Run("Extended/ok/CoreJS", func(t *testing.T) {
			compatMode := gojs.CompatibilityModeExtended
			b, err := getSimpleBuilder("/script.js",
		...
		t.Run("Base/ok/Minimal", func(t *testing.T) {
			compatMode := gojs.CompatibilityModeBase
			b, err := getSimpleBuilder("/script.js",
...
func TestBuilderEnv(t *testing.T) {
	...
	`
	b, err := getSimpleBuilder("/script.js", data, env, compatMode)
	if !assert.NoError(t, err) {
		return
	}
